...
const Sort = () => {
  ...
  const handlePriceClick = () => {
    setSort(Sorts.PRICE);
    dispatch(ActionCreator.setCurrentSort(isLowToHigh ? (a, b) => a.price - b.price : (a, b) => b.price - a.price));
  };

  const handlePopularityClick = () => {
    setSort(Sorts.POPULARITY);
    dispatch(ActionCreator.setCurrentSort(isLowToHigh ? (a, b) => a.reviews - b.reviews : (a, b) => b.reviews - a.reviews));
  };
  ...
  return (
    <section className="sort">
      ...
      <button className={`sort__btn${!isLowToHigh ? ` sort__btn--current-direction` : ``} button`} type="button" onClick={() => handleDownDirectionClick()}>
        <svg width="14" height="11" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.417.333H.584L7 10.416 13.417.333Z" fill="#6C6C6C" /></svg>
